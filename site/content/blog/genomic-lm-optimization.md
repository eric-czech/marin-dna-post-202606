...
VEP is arguably where gLMs are most likely to matter first. A useful VEP model can help scale clinical interpretation for rare disease, hereditary cancer, and variants of uncertain significance.[^vep-clinical] It can also help connect genetic association signals to disease mechanisms, target selection, and causal-variant prioritization in GWAS fine-mapping.[^vep-therapeutic] The same kind of evidence is relevant to clinical trial design when genetics can inform patient stratification, enrollment criteria, or mechanism-based cohort definition.[^vep-trials] These are already high-value decisions across medicine and drug discovery, and they are often limited by sparse labels, incomplete assays, and uneven functional annotation. That makes VEP a natural test for unsupervised genomic modeling. If a model has learned useful sequence-level constraints from DNA alone, it should help rank variants in places where direct experimental evidence is weak or nonexistent.
