...
[^metazoan-token-pool]: The proportional mix used here spans CDS, upstream, and downstream regions, with ~331M training examples and ~85B tokens from 366,411 RefSeq accessions across ~500 animal species. Each region is cycled only once, which keeps the transfer validation from being tangled up with epoching effects.
